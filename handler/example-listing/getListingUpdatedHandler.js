...
const { PENDING_APPROVAL_STATE } = require("../../config/example-listing");
...
module.exports = async (event) => {
  ...
  try {
    ...
    console.log(
      `Handle event [${LISTING_UPDATED}] listing (id: ${listingId.uuid}) successfully`
    );
  } catch (error) {
    console.error(
      `Handle event [${LISTING_UPDATED}] listing (id: ${listingId.uuid}) error`,
      error
    );
  }
};
